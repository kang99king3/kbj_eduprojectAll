...
public class HkController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("boardlist")){//글목록 요청처리
			...
			//한번에 출력될 페이지 행의개수 요청이 없는지,있는지 판단해서 rcount를 세션에 저장
			if(rcount==null) {
				rcount=(String)request.getSession().getAttribute("rcount");
			}else {
				request.getSession().setAttribute("rcount", rcount);				
			}
			...
			List<HkDto> list=dao.getAllList(pnum,rcount);//글목록 구하기
			int pcount=dao.getPCount(rcount);//페이지 개수 구하기
			
			Map<String, Integer>map=Paging.pagingValue(pcount, pnum, 5);
			
			request.setAttribute("pMap", map);//페이징처리를 위한 값 4개를 담은 객체 전달
